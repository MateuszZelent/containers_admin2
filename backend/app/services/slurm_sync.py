...
class SlurmSyncService:
            ...
            for db_job in db_jobs:
                if db_job.job_id in slurm_jobs_map:
                    ...
                    if slurm_state != db_job.status:
                        ...
                        slurm_logger.info(
                            f"Updated job {db_job.job_id} status: {old_status} -> {slurm_state}"
                        )

                    # Update node if available
                    if (
                        "node" in slurm_job
                        and slurm_job["node"] != "(None)"
                        and slurm_job["node"] != db_job.node
                    ):
                        db_job.node = slurm_job["node"]
                        slurm_logger.info(
                            f"Updated job {db_job.job_id} node: {db_job.node}"
                        )
                    ...
                    if db_job.status != "COMPLETED":
                        ...
                        db_job.updated_at = datetime.datetime.utcnow()
                        slurm_logger.info(
                            f"Marked job {db_job.job_id} as COMPLETED (not found in SLURM)"
                        )
                        self.db.add(db_job)
                    ...
                    if job_name.startswith("container_"):
                        ...
                        if len(parts) > 1:
                            ...
                            if possible_username in username_to_user:
                                ...
                                self.db.add(new_job)
                                slurm_logger.info(
                                    f"Added new job {slurm_job_id} from SLURM for user {possible_username}"
                                )
    ...
    async def _check_completed_jobs(self):
        ...
        try:
            ...
            slurm_logger.debug(
                f"Checking {len(active_jobs)} potentially completed jobs"
            )
            ...
            for job in active_jobs:
                if job.job_id not in active_slurm_ids:
                    ...
                    slurm_logger.info(
                        f"Marked job {job.job_id} as COMPLETED (verification)"
                    )
                    self.db.add(job)
