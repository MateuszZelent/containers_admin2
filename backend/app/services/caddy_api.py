...
#                 logger.error(f"Failed to add route for {domain}: {response.status_code} - {response.text}")
#                 return False
            
#             # Add domain to TLS subjects for automatic certificate
#             tls_response = requests.get(f"{self.api_url}/config/apps/tls/automation/policies/0/subjects")
...
#                         logger.error(f"Failed to add {domain} to TLS subjects: {tls_update.status_code} - {tls_update.text}")
#                         return False
#             else:
#                 logger.error(f"Failed to get TLS subjects: {tls_response.status_code} - {tls_response.text}")
#                 return False
                
#             logger.info(f"Successfully configured domain {domain} with TLS and reverse proxy to {target}:{target_port}")
#             return True
...
#         try:
#             # Get current routes
#             response = requests.get(f"{self.api_url}/config/apps/http/servers/srv0/routes")
#             if response.status_code != 200:
...
#                 logger.error(f"Failed to update routes: {update_response.status_code} - {update_response.text}")
#                 return False
...
#                     )
#                     if tls_update.status_code != 200:
#                         logger.error(f"Failed to remove {domain} from TLS subjects: {tls_update.status_code} - {tls_update.text}")
#                         return False
#             else:
#                 logger.error(f"Failed to get TLS subjects: {tls_response.status_code} - {tls_response.text}")
#                 return False
                
#             logger.info(f"Successfully removed domain {domain} from Caddy configuration")
#             return True
