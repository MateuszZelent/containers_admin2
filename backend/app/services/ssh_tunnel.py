...
class SSHTunnelService:
    ...
    async def restore_active_tunnels(self) -> Dict[str, int]:
        ...
        cluster_logger.info("Starting restoration of active tunnels after backend restart")
        ...
        for tunnel in active_tunnels:
            try:
                ...
                if not hasattr(tunnel, 'internal_port') or not tunnel.internal_port or \
                   not hasattr(tunnel, 'external_port') or not tunnel.external_port or \
                   not hasattr(tunnel, 'remote_port') or not tunnel.remote_port or \
                   not hasattr(tunnel, 'remote_host') or not tunnel.remote_host or \
                   not hasattr(tunnel, 'node') or not tunnel.node or \
                   not hasattr(tunnel, 'job_id') or not tunnel.job_id:
                    cluster_logger.warning(f"Tunnel id={tunnel.id} has missing required attributes - marking as DEAD")
                    tunnel.status = "DEAD"
                    failed_count += 1
                    continue
                    
                # Check if the ports are already in use
                internal_port_in_use = await self._is_port_in_use_async(tunnel.internal_port, check_external=False)
                external_port_in_use = await self._is_port_in_use_async(tunnel.external_port, check_external=True)
                
                # If both ports are in use, check if the tunnel is actually working
                if internal_port_in_use and external_port_in_use:
                    is_working = await self.test_tunnel(tunnel.external_port, tunnel.node)
                    if is_working:
                        cluster_logger.info(f"Tunnel id={tunnel.id} is already working - reattaching")
                        
                        # Try to get PIDs of the processes for tracking
                        ssh_pid = await self._get_pid_for_pattern(f"ssh.*{tunnel.internal_port}:{tunnel.node}:{tunnel.remote_port}")
                        socat_pid = await self._get_pid_for_pattern(f"socat.*TCP-LISTEN:{tunnel.external_port}")
                ...
                cluster_logger.info(f"Recreating tunnel id={tunnel.id} for job_id={tunnel.job_id}")
                ...
                if not job or not job.port or not job.node:
                    cluster_logger.warning(f"Job {tunnel.job_id} is missing or has insufficient data - marking tunnel as DEAD")
                    tunnel.status = "DEAD"
                    failed_count += 1
                    continue
                ...
                if not ssh_success:
                    cluster_logger.error(f"Failed to restore SSH tunnel for id={tunnel.id} - marking as DEAD")
                    tunnel.status = "DEAD"
                    failed_count += 1
                    continue
                ...
                if not socat_success:
                    ...
                    await self._kill_ssh_tunnel_async(tunnel.internal_port)
                    cluster_logger.error(f"Failed to restore socat forwarder for tunnel id={tunnel.id} - marking as DEAD")
                    tunnel.status = "DEAD"
                    failed_count += 1
                    continue
        ...
        # Log the summary
        cluster_logger.info(f"Tunnel restoration complete: {restored_count} restored, {failed_count} failed")
    ...
    async def _kill_tunnel_processes_async(self, tunnel):
        ...
        try:
            ...
            cluster_logger.error(f"Error killing processes for tunnel {tunnel.id}: {str(e)}")
    ...
    async def find_free_local_port(self) -> Optional[int]:
        ...
        cluster_logger.info(f"Ports registered in database after cleanup: {sorted(list(used_ports))}")
        ...
        if len(used_ports) >= (self.MAX_PORT - self.MIN_PORT):
            cluster_logger.warning("All ports are registered in database, checking if any are actually free")
            os_used_ports = set()
            ...
            if len(os_used_ports) < len(used_ports):
                cluster_logger.warning(f"Found discrepancy: {len(os_used_ports)} ports used by OS, {len(used_ports)} registered in DB")
                ...
                for tunnel in self.db.query(SSHTunnel).all():
                    if hasattr(tunnel, 'external_port') and tunnel.external_port:
                        if await self._is_port_in_use_async(tunnel.external_port, check_external=True):
                            used_ports.add(tunnel.external_port)
                    if hasattr(tunnel, 'internal_port') and tunnel.internal_port:
        ...
        max_time = 30  # seconds
        attempts = 0
        max_attempts = min(self.MAX_PORT - self.MIN_PORT, 100)  # Limit based on range size
        ...
        while time.time() - start_time < max_time and attempts < max_attempts:
            ...
            if not all_ports:
                cluster_logger.warning(f"All ports in range {self.MIN_PORT}-{self.MAX_PORT} have been checked and are in use")
                cluster_logger.warning(f"Ports in use: {sorted(list(ports_in_use))}")
                cluster_logger.warning(f"Free ports that were otherwise excluded: {sorted(list(free_ports))}")
                return None
            ...
            if port in used_ports:
                cluster_logger.debug(f"Port {port} already registered in database - skipping")
                continue
        ...
        cluster_logger.error(f"Could not find free port after {attempts} attempts and {time.time() - start_time:.2f} seconds")
        ...
        cluster_logger.error(f"- Free ports that were excluded for other reasons: {len(free_ports)}")
    ...
    async def _is_port_in_use_async(self, port: int, check_external: bool = True) -> bool:
        ...
        if result:
            cluster_logger.debug(f"Port {port} is in use on {'0.0.0.0' if check_external else '127.0.0.1'}")
        
        return result

    async def _establish_ssh_tunnel_async(self, local_port: int, remote_port: int, remote_host: str, node: str) -> Tuple[bool, Optional[int]]:
        ...
        try:
            ...
            if not await self._is_port_in_use_async(local_port, check_external=False):
                cluster_logger.error(f"SSH tunnel port {local_port} is not listening after start attempt")
                return False, None
                
            # Try to get the PID of the SSH process
            pid = await self._get_pid_for_pattern(f"ssh.*{local_port}:{node}:{remote_port}")
            
            if pid:
                cluster_logger.debug(f"SSH tunnel established with PID {pid}")
                return True, pid
            else:
                cluster_logger.debug(f"SSH tunnel established, but couldn't determine PID")
                return True, None
        ...
        try:
            ...
            if local_port in self._processes:
                ...
                try:
                    ...
                    cluster_logger.warning(f"Error killing SSH tunnel with PID {pid}: {str(e)}")
            ...
            if pid:
                ...
                await process.communicate()
                cluster_logger.debug(f"Killed SSH tunnel on port {local_port} with PID {pid}")
            else:
    ...
    async def get_or_create_tunnel(self, job: Job) -> Optional[SSHTunnelInfo]:
        ...
        if existing_tunnel:
            ...
            cluster_logger.debug(f"Found existing tunnel id={existing_tunnel.id} for job {job.id}")
            is_working = False
            ...
            if is_working:
                ...
                cluster_logger.warning(f"Existing tunnel for job {job.id} is not working - cleaning up")
    ...
    async def _start_socat_forwarder_async(self, external_port: int, internal_port: int) -> Tuple[bool, Optional[int]]:
        ...
        try:
            ...
            if await self._is_port_in_use_async(external_port, check_external=True):
                cluster_logger.warning(f"Port {external_port} is already in use despite earlier checks")
                ...
                if await self._is_port_in_use_async(external_port, check_external=True):
                    cluster_logger.error(f"Port {external_port} still in use after attempted cleanup")
                    return False, None
            ...
            if not await self._is_port_in_use_async(external_port, check_external=True):
                cluster_logger.warning(f"Socat started but port {external_port} is not listening")
                return False, None
            ...
            if not pid:
                ...
                pid = await self._get_pid_for_pattern(f"socat.*TCP-LISTEN:{external_port}")
            
            if pid:
                cluster_logger.info(f"Socat forwarder started with PID {pid} on port {external_port}")
            else:
                cluster_logger.info(f"Socat forwarder started on port {external_port} but couldn't determine PID")
    ...
    async def _kill_process_on_port_async(self, port: int):
        ...
        try:
            ...
            if pids and pids[0]:
                ...
                for pid in pids:
                    if pid.strip():
                        cluster_logger.debug(f"Killing process with PID {pid} using port {port}")
            ...
            if await self._is_port_in_use_async(port, check_external=True):
                cluster_logger.warning(f"Port {port} is still in use after kill attempt")
        except Exception as e:
        ...
        try:
            ...
            if port in self._processes:
                ...
                try:
                    ...
                    cluster_logger.warning(f"Error killing socat with PID {pid}: {str(e)}")
            ...
            await asyncio.sleep(0.5)
            if await self._is_port_in_use_async(port, check_external=True):
                cluster_logger.warning(f"Port {port} is still in use after killing socat")
            ...
            cluster_logger.error(f"Error while killing socat forwarder on port {port}: {e}")
    ...
    async def _emergency_cleanup_async(self):
        ...
        for tunnel in all_tunnels:
            ...
            if should_delete:
                cluster_logger.info(f"Emergency cleanup: Deleting tunnel id={tunnel.id}")
                await self._kill_tunnel_processes_async(tunnel)
                self.db.delete(tunnel)
                count += 1
