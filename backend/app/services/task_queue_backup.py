...
class TaskQueueService:
    ...
    def _get_monitor_lock(self):
        ...
        if self._monitor_lock is None:
            try:
                ...
                # We're not in an async context, create the lock anyway
                # It will be used later when we enter async context
                import warnings
                warnings.warn(
                    "Creating asyncio.Lock outside of async context. "
                    "This may cause issues if event loop changes.",
                    RuntimeWarning
                )
                # Create a new event loop temporarily to create the lock
                try:
                    ...
                    # If all else fails, create without loop
                    self._monitor_lock = asyncio.Lock()
    ...
    async def submit_task_to_slurm(self, task: TaskQueueJob) -> bool:
        ...
        try:
            ...
            # Job monitoring is handled by UnifiedSlurmMonitor
    ...
    def _estimate_progress(
        ...
        try:
            ...
            while True:
                # Use SLURM service directly for job status
                from app.services.slurm import SlurmSSHService

                # Get job details from SLURM
                try:
                    slurm_service = SlurmSSHService()
                    job_details = await slurm_service.get_job_details(slurm_job_id)

                    if job_details:
                        # Job is still active in SLURM
                        slurm_state = job_details.get("job_state", "UNKNOWN")
                        ...
                        progress = self._estimate_progress(
                            ...
                            {
                                "state": slurm_state,
                                "node": job_details.get("node_list", ""),
                                "time_used": job_details.get("run_time", ""),
                                "time_left": job_details.get("time_left", ""),
                            },
                        )
                    ...
                    cluster_logger.warning(f"Failed to get job details for {slurm_job_id}: {e}")
                    # Continue monitoring in case of temporary SLURM issues
