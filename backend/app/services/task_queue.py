...
class TaskQueueService:
    ...
    def get_task(self, task_id: Union[str, int]) -> Optional[TaskQueueJob]:
        ...
        if isinstance(task_id, int):
            return self.db.query(TaskQueueJob).filter(TaskQueueJob.id == task_id).first()
        return self.db.query(TaskQueueJob).filter(TaskQueueJob.task_id == task_id).first()
    
    def get_tasks(self, owner_id: int, status: Optional[str] = None, 
                  skip: int = 0, limit: int = 100) -> List[TaskQueueJob]:
    ...
    def _translate_path(self, filepath: str) -> str:
        ...
        if filepath and filepath.startswith("/mnt/local/kkingstoun/admin/pcss_storage/mannga"):
    ...
    def create_task(self, data: TaskQueueJobCreate, owner_id: int) -> TaskQueueJob:
        ...
        try:
            ...
            output_dir = os.path.join(
                settings.SIMULATION_OUTPUT_DIR,
                username,
                task_id
            )
    ...
    def update_task(self, task_id: Union[str, int], update_data: TaskQueueJobUpdate) -> Optional[TaskQueueJob]:
        ...
        try:
            ...
            if "status" in update_dict:
                ...
                elif new_status in [TaskStatus.COMPLETED, TaskStatus.ERROR, 
                                    TaskStatus.ERROR_RETRY_3, TaskStatus.CANCELLED]:
    ...
    def delete_task(self, task_id: Union[str, int], owner_id: int) -> bool:
        ...
        if task.status in [TaskStatus.CONFIGURING, TaskStatus.RUNNING] and task.slurm_job_id:
            try:
                asyncio.create_task(self.slurm_service.cancel_job(task.slurm_job_id))
            except Exception as e:
                cluster_logger.warning(f"Error cancelling task {task.task_id} in SLURM: {str(e)}")
    ...
    def get_queue_status(self, owner_id: Optional[int] = None) -> TaskQueueStatus:
        ...
        pending_tasks = query.filter(
            TaskQueueJob.status == TaskStatus.PENDING
        ).all()
    ...
    async def submit_task_to_slurm(self, task: TaskQueueJob) -> bool:
        ...
        try:
            ...
            # Submit to SLURM
            slurm_job_id = await self.slurm_service.submit_job(script_content, user.username)
    ...
    async def _process_queue_once(self):
        ...
        pending_tasks = self.db.query(TaskQueueJob).filter(
            TaskQueueJob.status == TaskStatus.PENDING
        ).order_by(
            TaskQueueJob.priority.desc(),
            TaskQueueJob.created_at.asc()
        ).limit(10).all()  # Process 10 tasks at a time
    ...
    async def _process_retries(self):
        ...
        now = datetime.utcnow()
        retry_tasks = self.db.query(TaskQueueJob).filter(
    ...
    def _start_job_monitor(self, task_id: int, slurm_job_id: str):
        ...
        monitor_task = asyncio.create_task(
            self._monitor_job(task_id, slurm_job_id)
        )
        ...
        try:
            ...
            while True:
                ...
                if job_info:
                    ...
                    new_status = self.SLURM_STATE_MAPPING.get(slurm_state, TaskStatus.UNKNOWN)
                    ...
                    # Update task status
                    if task.status != new_status or (progress is not None and task.progress != progress):
                        # Prepare update data
                        ...
                        if new_status == TaskStatus.RUNNING:
                            ...
                            check_interval = 30  # Check configuring jobs more frequently
                else:
                    ...
                    exit_code, final_state = await self._get_job_final_status(slurm_job_id)
                    ...
                    if exit_code == 0 and final_state in ["COMPLETED", "CD"]:
                        ...
                        if task.retry_count < 3:
                            ...
                            self.update_task(task_id, TaskQueueJobUpdate(
                                status=new_status,
                                error_message=f"SLURM job failed with exit code {exit_code}, state {final_state}",
                                exit_code=exit_code,
                                next_retry_at=next_retry_at,
                                progress=100 if new_status == TaskStatus.COMPLETED else task.progress
                            ))
                        else:
                            # No more retries
                            self.update_task(task_id, TaskQueueJobUpdate(
    ...
    def _estimate_progress(self, task: TaskQueueJob, job_info: Dict[str, Any]) -> Optional[int]:
    ...
    async def _get_job_final_status(self, slurm_job_id: str) -> Tuple[Optional[int], Optional[str]]:
        ...
        try:
            ...
            cluster_logger.error(f"Error getting final status for job {slurm_job_id}: {str(e)}")
            return None, None
    ...
    async def get_task_results(self, task: TaskQueueJob) -> Dict[str, Any]:
        ...
        if task.status not in [TaskStatus.COMPLETED, TaskStatus.ERROR, 
                              TaskStatus.ERROR_RETRY_1, TaskStatus.ERROR_RETRY_2, 
                              TaskStatus.ERROR_RETRY_3, TaskStatus.CANCELLED]:
            return {
        ...
        if task.status in [TaskStatus.ERROR, TaskStatus.ERROR_RETRY_1, 
                           TaskStatus.ERROR_RETRY_2, TaskStatus.ERROR_RETRY_3]:
        ...
        if task.status == TaskStatus.COMPLETED and task.output_dir:
            ...
            if os.path.exists(output_dir):
                ...
                output_files = [
                    str(path) for path in Path(output_dir).glob("*")
                    if path.is_file()
                ]
                ...
                if task.results_file:
                    ...
                    if os.path.exists(results_file):
                        try:
                            ...
                            result["results_error"] = f"Error reading results file: {str(e)}"
            else:
    ...
    async def cancel_task(self, task_id: Union[str, int], owner_id: int) -> bool:
        ...
        if task.status not in [TaskStatus.PENDING, TaskStatus.CONFIGURING, TaskStatus.RUNNING]:
            raise HTTPException(
        ...
        try:
            ...
            if task.slurm_job_id and task.status in [TaskStatus.CONFIGURING, TaskStatus.RUNNING]:
                await self.slurm_service.cancel_job(task.slurm_job_id)
                
            # Update task status
            self.update_task(task.id, TaskQueueJobUpdate(
