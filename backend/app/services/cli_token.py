...
class CLITokenService:
    ...
    def create_token(
        ...
        expires_at = datetime.utcnow() + timedelta(
            days=token_data.expires_days or 30
        )
    ...
    def update_token(
        ...
        cluster_logger.info(
            f"Updated CLI token {token_id} for user {user_id}"
        )
    ...
    def deactivate_token(self, token_id: int, user_id: int) -> bool:
        ...
        cluster_logger.info(
            f"Deactivated CLI token {token_id} for user {user_id}"
        )
    ...
    def delete_token(self, token_id: int, user_id: int) -> bool:
        ...
        cluster_logger.info(
            f"Deleted CLI token {token_id} for user {user_id}"
        )
