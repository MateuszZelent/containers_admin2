...
class SSHTunnelServiceOptimized:
    ...
    async def _find_process_by_command(self, pattern: str) -> Optional[int]:
        ...
        try:
            ...
            cluster_logger.error(f"Error finding process by pattern '{pattern}': {str(e)}")
            return None
    ...
    async def _find_free_port(self) -> Optional[int]:
        ...
        used_ports = set()
        for tunnel in self.db.query(SSHTunnel).filter(
            SSHTunnel.status.in_([TunnelStatus.ACTIVE.value])
        ).all():
            if tunnel.external_port:
