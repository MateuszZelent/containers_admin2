...
async def get_monitor_status(current_user: User = Depends(get_current_superuser)):
    ...
    return {
        ...
        "metrics": {
            ...
            "last_slurm_sync": (
                ...
                if monitor.metrics.last_slurm_sync
                else None
            ),
...
async def restart_monitor(current_user: User = Depends(get_current_superuser)):
...
async def get_port_allocations(current_user: User = Depends(get_current_superuser)):
