...
import { Clock, Save, RefreshCcw, AlertCircle } from "lucide-react";
...
export function ResourceMonitoringSettings() {
  const [settings, setSettings] = useState<MonitoringSettings | null>(null);
  const [selectedInterval, setSelectedInterval] = useState<string>("10");
  ...
  const intervalOptions = [
    ...
    { value: "5", label: "Co 5 minut", description: "Częste monitorowanie" },
    { value: "10", label: "Co 10 minut", description: "Standardowe monitorowanie (zalecane)" },
    { value: "30", label: "Co 30 minut", description: "Rzadkie monitorowanie" },
    { value: "60", label: "Co godzinę", description: "Bardzo rzadkie monitorowanie" },
  ];
  ...
  const fetchSettings = async () => {
    try {
      ...
      toast.error("Nie udało się pobrać ustawień monitoringu");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const saveSettings = async () => {
    try {
      ...
      await adminApi.updateMonitoringInterval(newInterval);
      ...
      toast.success(`Częstotliwość monitoringu zmieniona na ${intervalOptions.find(opt => opt.value === selectedInterval)?.label.toLowerCase()}`);
    } catch (error) {
  ...
  const restartMonitoring = async () => {
    try {
      ...
      await adminApi.restartMonitoring();
      
      toast.success("Monitoring zasobów został zrestartowany z nowymi ustawieniami");
      await fetchSettings();
    } catch (error) {
      console.error("Failed to restart monitoring:", error);
      toast.error("Nie udało się zrestartować monitoringu");
    } finally {
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center py-8">
        ...
        <span className="text-sm text-muted-foreground">Ładowanie ustawień...</span>
      </div>
    );
  }
      ...
      <Card className="bg-red-50 dark:bg-red-950/20 border-red-200 dark:border-red-800">
        <CardContent className="p-4">
          <div className="flex items-center gap-2 text-red-700 dark:text-red-400">
            ...
            <span className="text-sm">Nie udało się załadować ustawień monitoringu</span>
          </div>
        </CardContent>
      </Card>
    );
      ...
      <Card className="bg-slate-50 dark:bg-slate-900/50">
        <CardContent className="p-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
            ...
            <div className="flex items-center gap-2">
              <span className="font-medium">Zebranych pomiarów:</span>
              <span>{settings.total_snapshots}</span>
            </div>
          </div>
          {settings.last_snapshot && (
            <div className="mt-3 pt-3 border-t border-slate-200 dark:border-slate-700 text-xs text-muted-foreground">
              Ostatni pomiar: {new Date(settings.last_snapshot).toLocaleString('pl-PL')}
            </div>
          )}
        </CardContent>
      </Card>
            ...
            <CardContent className="p-4">
              <div className="flex items-start gap-2 text-amber-700 dark:text-amber-400">
                ...
                <div className="text-sm">
                  ...
                  <div>Monitorowanie co minutę generuje dużo danych i może wpłynąć na wydajność. Używaj tylko do testów.</div>
                </div>
              </div>
            </CardContent>
          </Card>
